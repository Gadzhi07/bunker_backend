...
Revision ID: 001
Revises: 
Create Date: 2024-08-05 17:38:15.604665
...
def upgrade() -> None:
    ...
    sa.Enum('BunkerCard', 'DangerCard', name='bunker_card_type_enum').create(op.get_bind())
    sa.Enum('kicked', 'not_kicked', name='team_side_enum').create(op.get_bind())
    sa.Enum('wait', 'rounds', 'advanced', 'finished', name='game_status_enum').create(op.get_bind())
    sa.Enum('Basic', 'Advanced', name='game_mode_enum').create(op.get_bind())
    sa.Enum('profession', 'biology', 'health', 'hobby', 'baggage', 'facts', 'phobia', 'character_trait', name='character_card_type_enum').create(op.get_bind())
    ...
    sa.Column('game_mode', postgresql.ENUM('Basic', 'Advanced', name='game_mode_enum', create_type=False), server_default='Basic', nullable=False),
    ...
    sa.Column('card_type', postgresql.ENUM('profession', 'biology', 'health', 'hobby', 'baggage', 'facts', 'phobia', 'character_trait', name='character_card_type_enum', create_type=False), nullable=False),
    ...
    sa.Column('status', postgresql.ENUM('wait', 'rounds', 'advanced', 'finished', name='game_status_enum', create_type=False), server_default='wait', nullable=False),
    sa.Column('current_queue', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['current_queue'], ['user_gamedata.id'], name='games_current_queue_fk', use_alter=True),
    ...
    sa.Column('team_side', postgresql.ENUM('kicked', 'not_kicked', name='team_side_enum', create_type=False), server_default='not_kicked', nullable=False),
    sa.Column('bunker_card_type', postgresql.ENUM('BunkerCard', 'DangerCard', name='bunker_card_type_enum', create_type=False), nullable=False),
    ...
    sa.Column('type_of_card_to_open', postgresql.ENUM('profession', 'biology', 'health', 'hobby', 'baggage', 'facts', 'phobia', 'character_trait', name='character_card_type_enum', create_type=False), nullable=True),
    sa.ForeignKeyConstraint(['game_id'], ['games.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('votes_rounds',
    sa.Column('round_id', sa.Integer(), nullable=False),
    sa.Column('voting_id', sa.Integer(), autoincrement=True, nullable=False),
    ...
    op.create_foreign_key("games_current_queue_fk", "games", "user_gamedata", ["current_queue"], ["id"])


def downgrade() -> None:
    op.drop_constraint('games_current_queue_fk', 'games')
    ...
    op.drop_table('users')
    sa.Enum('profession', 'biology', 'health', 'hobby', 'baggage', 'facts', 'phobia', 'character_trait', name='character_card_type_enum').drop(op.get_bind())
    sa.Enum('Basic', 'Advanced', name='game_mode_enum').drop(op.get_bind())
    sa.Enum('wait', 'rounds', 'advanced', 'finished', name='game_status_enum').drop(op.get_bind())
    sa.Enum('kicked', 'not_kicked', name='team_side_enum').drop(op.get_bind())
    sa.Enum('BunkerCard', 'DangerCard', name='bunker_card_type_enum').drop(op.get_bind())
    # ### end Alembic commands ###
